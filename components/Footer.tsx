    ...
    <footer style={{ background: '#08090F', padding: 'clamp(3rem,6vw,7rem) var(--px) 0', borderTop: '1px solid rgba(201,168,76,0.12)', paddingBottom: 'calc(80px + env(safe-area-inset-bottom, 0px))' }}>
      ...
      <div className="inner footer-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '4rem', paddingBottom: '4rem', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div>
          ...
          <p style={{ fontSize: '0.88rem', lineHeight: 1.85, color: 'rgba(255,255,255,0.58)', fontWeight: 300, maxWidth: '260px' }}>
            A Nevada non-profit raising awareness and funds to battle Huntington&apos;s Disease — one round of golf at a time.
          </p>
          <div style={{ marginTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <a href="tel:7756918860" style={{ color: 'rgba(255,255,255,0.72)', textDecoration: 'none', fontSize: '0.88rem' }}>📞 775-691-8860</a>
            <a href="mailto:info@nvforhd.com" style={{ color: 'rgba(255,255,255,0.72)', textDecoration: 'none', fontSize: '0.88rem' }}>✉ info@nvforhd.com</a>
          </div>
          <p style={{ marginTop: '1.2rem', fontSize: '0.68rem', color: 'rgba(255,255,255,0.2)' }}>
        ...
        <div>
          ...
          <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {[['/', 'Home'], ['/about', 'About Us'], ['/causes', 'Causes'], ['/gallery', 'Gallery'], ['/sponsors', 'Sponsors'], ['/contact', 'Contact']].map(([href, label]) => (
              <li key={href}><Link href={href} style={{ color: 'rgba(255,255,255,0.55)', textDecoration: 'none', fontSize: '0.85rem', fontWeight: 300 }}>{label}</Link></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <li><Link href="/terms" style={{ color: 'rgba(255,255,255,0.55)', textDecoration: 'none', fontSize: '0.85rem', fontWeight: 300 }}>Terms &amp; Conditions</Link></li>
        ...
        <div>
          ...
          <a href={DONATE} target="_blank" rel="noopener" className="btn-gold" style={{ display: 'block', textAlign: 'center', marginBottom: '0.65rem', fontSize: '0.68rem', padding: '0.75rem 1rem' }}>Donate Now →</a>
